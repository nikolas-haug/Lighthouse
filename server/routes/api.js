...
router.get('/api/providers/:keyword/:location/:USstate', function(req, res) {
    // make the api query with user params set from services component client side
    axios.get(`${URL}?q=${req.params.keyword}&location=${req.params.USstate}-${req.params.location}&skip=0&limit=10&user_key=${KEY}`)
        .then((response) => {
